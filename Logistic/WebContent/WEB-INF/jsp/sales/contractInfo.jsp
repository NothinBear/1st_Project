...
<div class="modal fade" id="amountModal" role="dialog">
    ...
    <script>
        const contractSearchBtn = document.querySelector("#contractSearchButton");				//견적조회
        const myGrid = document.querySelector("#myGrid");
        const myGrid2 = document.querySelector("#myGrid2");
        const fromDate = document.querySelector("#fromDate");
        const toDate = document.querySelector("#toDate");
        const pdfOpenBtn = document.querySelector("#pdfOpenButton");							//O pdf출력
        const contractDetaillBtn = document.querySelector("#contractDetaillButton");			//O 견적상세조회
 ...
 // o if customer modal hide, next cell
    ...
    function setDataOnCustomerName() {
        ...
        pdfOpenBtn.addEventListener("click", () => {
        	...
        	window.open("${pageContext.request.contextPath}/logisticsInfo/report.html?method=contractReport&orderDraftNo=" + getRowIdValue,"window", "width=1600, height=1000" );
        });
        // 수주상세 grid
    ...
    function addRow(event) {
        if (event.innerText == "견적상세추가") {
        	...
        	contractDetailInfoGridOptions.api.updateRowData({add: [newRow]});
        } 
    }

    function deleteRow(event) {
        let selected = contractDetailInfoGridOptions.api.getFocusedCell();                   
        ...
        if (event.innerText == "선택한견적상세삭제"){
                console.log("견적상세삭제");
                let selectedRows   = contractDetailInfoGridOptions.api.getSelectedRows();
                console.log("선택된 행" + selectedRows );
                selectedRows.forEach( function(selectedRow, index) {
                    console.log(selectedRow);
                    detailItemCode.splice(detailItemCode.indexOf(selectedRow.itemCode), 1); // 배열요소 제거
                    contractDetailInfoGridOptions.api.updateRowData({remove: [selectedRow]});
                });
        }
    }
