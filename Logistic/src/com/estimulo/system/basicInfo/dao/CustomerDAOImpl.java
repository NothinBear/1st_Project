...
public class CustomerDAOImpl implements CustomerDAO {
	...
	public ArrayList<CustomerTO> selectCustomerListByCompany() {
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setProducedProduct(rs.getString("PRODUCED_PRODUCT"));
	...
	public ArrayList<CustomerTO> selectCustomerListByItem(String itemGroupCode) {
		...
		if (logger.isDebugEnabled()) {
			logger.debug("CustomerDAOImpl : selectCustomerListByItem 시작");
		}
		...
		try {
			...
			query.append("select i.item_code\r\n"
					+ "			,i.item_name\r\n"
					+ "			,i.item_group_code\r\n"
					+ "			,c.customer_name\r\n"
					+ "			,c.customer_code\r\n"
			...
			if(itemGroupCode!=null) {
				query.append("and i.item_group_code=?");
			}
			pstmt = conn.prepareStatement(query.toString());
			if(itemGroupCode!=null) {
				pstmt.setString(1, itemGroupCode);
			}
			rs = pstmt.executeQuery();
			...
			while (rs.next()) {
				...
				bean.setItemGroupName(rs.getString("ITEM_GROUP_CODE"));
				bean.setCustomerName(rs.getString("CUSTOMER_NAME"));
				bean.setCustomerCode(rs.getString("CUSTOMER_CODE"));
