...
public HashMap<String, Object> insertMrpList(String mrpRegisterDate) {
	if (logger.isDebugEnabled()) {
        logger.debug("MrpDAOImpl : insertMrpList 시작");
     }
     ...
     try {
        conn = dataSourceTransactionManager.getConnection();
        StringBuffer query = new StringBuffer();
        query.append(" {call P_INSERT_MRP(?,?,?,?)} ");
        System.out.println("      @ 프로시저 호출");
        
         cs = conn.prepareCall(query.toString());
         cs.setString(1, mrpRegisterDate);
           cs.registerOutParameter(2,OracleTypes.NUMBER);
           ...
           if(rs.next()) {
        	   resultMap.put("firstMrpNo",rs.getString("FIRST_NO"));
        	   resultMap.put("lastMrpNo",rs.getString("LAST_NO"));
        	   resultMap.put("length",rs.getString("LENG"));
           }
        resultMap.put("errorCode",errorCode);
        resultMap.put("errorMsg",errorMsg);
