...
public class ContractDetailDAOImpl implements ContractDetailDAO {
	...
	public HashMap<String,Object> insertContractDetail(HashMap<String,String>  workingContractList) {
		...
		try {
			...
			query.append(" {call P_REGISTER_NEW_CONTRACT(?,?,?,?,?,?,?,?,?,?)} ");

			cs = conn.prepareCall(query.toString());
			
			cs.setString(1,workingContractList.get("estimateNo"));
			cs.setString(2,workingContractList.get("contractType"));
			cs.setString(3,workingContractList.get("contractRequester"));
			cs.setString(4,workingContractList.get("personCodeInCharge"));
			cs.setString(5,workingContractList.get("discription"));
			cs.setString(6,workingContractList.get("newContractNo"));
			cs.setString(7,workingContractList.get("customerCode"));
			...
			System.out.println("      @ 에러메세지"+errorMsg);

			while (rs.next()) {
				contractDetailNoArr.add(rs.getString("CONTRACT_DETAIL_NO"));
		    }
	...
	public ArrayList<ContractDetailInMpsAvailableTO> selectContractDetailListInMpsAvailable(String searchCondition,
		...
		try {
			...
			query.append("select\r\n" + 
					...
					"AND operation_completed_status IS NULL "// 작업완료여부. IS NULL 을 검사하는 이유는 즉시납품이면 둘다 Y 이고 아니면 MPS 부터 단계를 거쳐가야하기 때문이다.
					);
