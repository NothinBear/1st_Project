...
const getListData = (divisionCodeNo) => {
    ...
    xhr.open('POST',
        '/logisticsInfo/codeList.do?method=findDetailCodeList&divisionCodeNo='+divisionCodeNo,
        true);
...
const getStandardUnitPrice = (itemCode, unit) => {
    ...
    // BOX
    xhr.open('POST',
        `/logisticsInfo/getStandardUnitPrice.do?method=getStandardUnitPriceBox&itemCode=${itemCode}`,
        true);
    xhr.setRequestHeader('Accept', 'application/json');
    xhr.send();
    xhr.onreadystatechange = () => {
        ...
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            let resultMsg =
                ...
                + txt.result.firstMrpNo + ` 부터 ` + txt.result.lastMrpNo + ` 까지 <br>
                     총 ` + (txt.result.length-1) + `건 등록 완료.<br>`) + `<br>
                     위와 같이 작업이 처리되었습니다.`;
