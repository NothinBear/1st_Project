 ...
 // o if customer modal hide, next cell
    ...
    function setDataOnCustomerName() {
        ...
        pdfOpenBtn.addEventListener("click", () => {
           ...
           window.open("${pageContext.request.contextPath}/base/report.html?method=estimateReport&orderDraftNo="+getRowIdValue,"window", "width=1600, height=1000")
        });

      //send Email
        pdfsend.addEventListener("click",()=>{
        	let selectedNodes = estimuloInfoGridOptions.api.getSelectedNodes();
        	...
        	if (selectedNodes == "") {
        	 ...
        	 let estimateNo = selectedNodes[0].estimateNo;
        	 console.log(estimateNo);
        	 Swal.fire({
        	      title: "메일을 보내시겠습니까?",
        	      icon: 'warning',
        	      ...
        	      if (result.isConfirmed) {
        	      let xhr = new XMLHttpRequest();
        	      xhr.open('POST', '${pageContext.request.contextPath}/base/SendEmail.do' +
        	      ...
        	      xhr.onreadystatechange = () => {
        	        if (xhr.readyState == 4 && xhr.status == 200) {
        	          let resultMsg = "보낸 메일함을 확인해 주세요"
        	          Swal.fire({
        	            title: "<h5>견적 메일 전송이 완료되었습니다.</h5>",
        	            html: resultMsg,
        	            icon: "success",
        	          });
        	        }
