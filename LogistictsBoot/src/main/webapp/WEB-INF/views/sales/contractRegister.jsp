...
<script>
  ...
  let contractColumn = [
    ...
    {headerName: "수주유형", field: "contractTypeName", editable: true},
    ...
    {headerName: "견적담당자", field: "estimateRequester"},
  ...
  function setDataOnCustomerName() {
    ...
    console.log('setDataValue("contractType", to.detailCode)')
    contractRowNode.setDataValue("contractType", to.detailCode);
    contractRowNode.setDataValue("contractTypeName", to.detailCodeName);
  ...
  let estDetailGridOptions = {
    ...
    defaultColDef: {editable: false, resizable : true},
    overlayNoRowsTemplate: "상세조회 버튼으로 내용을 조회해주세요.",
  ...
  contractBatchInsertBtn.addEventListener("click", () => {
    ...
    newEstimateRowValues.map(selectedData => {
     ...
     Swal.fire({
      ...
      xhr.onreadystatechange = () => {
        if (xhr.readyState == 4 && xhr.status == 200) {
          ...
          let conDNStr="";
          const conDetailList = Object.values(txt.gridRowJson);
          for(let i=0;i<conDetailList[0].length;i++){
        	  if(conDetailList[i]!=undefined){
        		  conDNStr+=conDetailList[0][i].contractDetailNo;
            	  conDNStr+="<br>"
            	  console.log(conDetailList[0][i].contractDetailNo);
        	  }
          }
