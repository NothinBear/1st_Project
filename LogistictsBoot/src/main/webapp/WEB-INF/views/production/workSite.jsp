...
<script>
 ...
 // const workSiteRawMaterialsBtn = document.querySelector("#workSiteRawMaterials");  // 원재료검사
  ...
  workSiteListBtn.addEventListener('click', () => {  // 조회
    getWorkSiteList();
  });
 ...
 /*  workSiteRawMaterialsBtn.addEventListener('click', () => {
  ...
  // o Production
  workSiteProductionBtn.addEventListener('click', () => {  // 제품제작 
    if (isCheck()) {
      return;
    }
    let selectedRows = workSiteGridOptions.api.getSelectedRows();
  ...
  // o SiteExamine
  workSiteExamineBtn.addEventListener('click', () => {  // 판매제품검사 
    if (isCheck()) {
      return;
    }
    let selectedRows = workSiteGridOptions.api.getSelectedRows();
  ...
  // o 모의전개된 결과 작업지시
  completeBtn.addEventListener('click', () => { // 검사 및 제작완료 
    let displayModel = workSiteSimulationGridOptions.api.getModel();
    let modalData = displayModel.gridApi.getRenderedNodes();   // 랜더링된 노드들을 가지고 오는 방식인듯 ? 
